    ...
    headerSearch.addEventListener('input', (e) => {
      ...
      filtered.forEach(asset => {
        ...
        tr.innerHTML = `
          ...
          <td class="py-2.5 px-1.5 lg:px-2 pr-3 text-center row-actions bg-white w-[150px] min-w-[150px] max-w-[150px] overflow-hidden"><div class="flex items-center justify-center gap-1.5 flex-nowrap w-full max-w-full mx-auto px-0.5"><button onclick="event.stopPropagation(); openRaioXForTicker('${asset.ticker}')" class="inline-flex items-center justify-center w-8 h-8 rounded-md bg-[rgba(6,182,212,0.08)] hover:bg-[rgba(6,182,212,0.12)] text-[#0891b2] border border-[rgba(6,182,212,0.15)] transition-colors duration-150 active:scale-95 shrink-0"><i data-lucide="microscope" class="shrink-0" style="width:11px;height:11px"></i></button><button onclick="event.stopPropagation(); openEditAsset('${asset.ticker}')" class="inline-flex items-center justify-center w-8 h-8 rounded-md text-[11px] font-bold bg-[#00513f] hover:bg-[#006b55] text-white shadow-sm border border-[#00513f] transition-all duration-150 active:scale-95 shrink-0"><i data-lucide="pencil" class="shrink-0" style="width:11px;height:11px"></i></button><button onclick="event.stopPropagation(); removeAssetFromPortfolio('${asset.ticker}')" title="Remover ativo" class="inline-flex items-center justify-center w-8 h-8 rounded-md bg-white hover:bg-[#ffdad6] text-[#3e4945] hover:text-[#ba1a1a] border border-[rgba(190,201,195,0.4)] hover:border-[#ba1a1a]/20 transition-colors duration-150 active:scale-95 shrink-0"><i data-lucide="trash-2" class="shrink-0" style="width:11px;height:11px"></i></button></div></td>`;
...
function renderAssetsTable() {
  ...
  filtered.forEach(asset => {
    ...
    tr.innerHTML = `
      <td class="py-2.5 px-1.5 lg:px-2">
        <div class="flex items-center gap-2">
          ...
          <div class="min-w-0">
            ...
            <div class="text-[10px] lg:text-[11px] text-[#3e4945]/70 truncate max-w-[90px] lg:max-w-[130px]">${asset.name}</div>
          </div>
        </div>
      </td>
      ...
      <td class="py-3 px-2 pr-3 text-center row-actions bg-white w-[150px] min-w-[150px] max-w-[150px] overflow-hidden">
        <div class="flex items-center justify-center gap-1.5 flex-nowrap w-full max-w-full mx-auto px-0.5">
           <button onclick="event.stopPropagation(); openRaioXForTicker('${asset.ticker}')" title="Raio-X detalhado (composição e imóveis via Investidor10/StatusInvest)" class="inline-flex items-center justify-center w-8 h-8 rounded-md bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-600 border border-cyan-500/20 transition-colors duration-150 active:scale-95 focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500/40 shrink-0">
             <i data-lucide="microscope" class="shrink-0" style="width:12px;height:12px"></i>
           </button>
           <button onclick="event.stopPropagation(); openEditAsset('${asset.ticker}')" title="Editar ativo (quantidade, preço, proventos) – clique na linha também edita" class="inline-flex items-center justify-center w-8 h-8 rounded-md text-[11px] font-bold bg-indigo-600 hover:bg-indigo-500 text-white shadow-sm border border-indigo-600 transition-all duration-150 active:scale-95 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/40 shrink-0">
             <i data-lucide="pencil" class="shrink-0" style="width:11px;height:11px"></i>
           </button>
           <button onclick="event.stopPropagation(); removeAssetFromPortfolio('${asset.ticker}')" title="Excluir ativo da carteira" class="inline-flex items-center justify-center w-8 h-8 rounded-md bg-white hover:bg-rose-50 text-gray-500 hover:text-rose-500 border border-gray-200 hover:border-rose-200 transition-colors duration-150 active:scale-95 focus:outline-none focus-visible:ring-2 focus-visible:ring-rose-500/30 shrink-0">
